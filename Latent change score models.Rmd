...
semPlot::semPaths(fitBLCS, "std", label.cex = .6, label.scale = FALSE, edge.label.cex = 0.75, layout = "tree",
                  intercepts = FALSE,
                  color = list(lat = viridis::viridis(3, begin = 0.3)[1], 
                               man = viridis::viridis(3, begin = 0.3)[2], 
                               int = viridis::viridis(3, begin = 0.3)[3]))
...
```{r mediation}
data_mediation <- df %>% dplyr::select(X = intervention, M = PA_autonomous_diff, Y = padaysLastweek_diff)
