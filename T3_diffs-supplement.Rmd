...
# Centre the variable:
df_randeff_plot_rstanarm <- df %>% dplyr::mutate(mvpaAccelerometer_diff_centred = mvpaAccelerometer_diff - mean(mvpaAccelerometer_diff, na.rm = TRUE))

m_1 <- stan_glmer(mvpaAccelerometer_diff_centred ~ (1 | group), data = df_randeff_plot_rstanarm, chains = 2, iter = 2000) 
pt <- ranef(m_1) 
...
# groupmeans <- df %>% dplyr::select(mvpaAccelerometer_diff, group) %>% group_by(group) %>% 
#   summarise(mean = mean(mvpaAccelerometer_diff, na.rm = TRUE)) %>% dplyr::filter(complete.cases(.))
...
# Centre the variable:
df_randeff_plot_brms <- df %>% dplyr::mutate(mvpaAccelerometer_diff_centred = 
                      mvpaAccelerometer_diff - mean(mvpaAccelerometer_diff, na.rm = TRUE))

m_1 <- brms::bf(mvpaAccelerometer_diff_centred ~ (1 | group)) 
...
df %>% ggplot2::ggplot(aes(x = group, y = mvpaAccelerometer_diff_centred)) +
...
# Centre the variable:
df_randeff_plot_brms_autonomous <- df %>% dplyr::mutate(PA_autonomous_diff_centred = PA_autonomous_diff - mean(PA_autonomous_diff, na.rm = TRUE))

m_1 <- brms::bf(PA_autonomous_diff_centred ~ (1 | group)) 
...
df_randeff_plot_brms_autonomous %>% ggplot2::ggplot(aes(x = group, y = PA_autonomous_diff_centred)) +
...
averageDayActivityPlot_gender <- df %>%  
  dplyr::select(girl, intervention, track,
                     "Sedentary behavior" = sitLieAccelerometer_diff, 
                     "Standing still" = standingAccelerometer_diff, 
                     "Light PA" = lpaAccelerometer_diff, 
                     "Moderate PA" = mpaAccelerometer_diff, 
                     "Vigorous PA" = vpaAccelerometer_diff) %>%
...
averageDayActivityPlot_gender <- df %>% 
  dplyr::mutate_at(vars(contains("Accelerometer"), -contains("weartime")), funs(./weartimeAccelerometer_diff)) %>% 
  dplyr::select(girl, intervention, track,
                     "Sedentary behavior" = sitLieAccelerometer_diff, 
                     "Standing still" = standingAccelerometer_diff, 
                     "Light PA" = lpaAccelerometer_diff, 
...
df <- df %>% dplyr::mutate(
                         mvpaAccelerometer_diff = mvpaAccelerometer_diff / 60, 
                         mvpaAccelerometer_diff = mvpaAccelerometer_diff / 60,
                         weartimeAccelerometer_diff = weartimeAccelerometer_diff / 60,
                         weartimeAccelerometer_diff = weartimeAccelerometer_diff / 60,
                         sitLieAccelerometer_diff = sitLieAccelerometer_diff / 60,
                         sitLieAccelerometer_diff = sitLieAccelerometer_diff / 60
                         )
# 
# df <- df %>% dplyr::mutate(
...
```{r selfrep-determinants-simple-diamondplot, eval = FALSE}
PA_ci_girls <- ci_girls %>% dplyr::filter(diamondlabels %in% names(scales_diff) & grepl("PA_", diamondlabels) & !grepl("bct", diamondlabels))

PA_ci_boys <- ci_boys %>% dplyr::filter(diamondlabels %in% names(scales_diff) & grepl("PA_", diamondlabels) & !grepl("bct", diamondlabels))

PA_ci_intervention <- ci_intervention %>% dplyr::filter(diamondlabels %in% names(scales_diff) & grepl("PA_", diamondlabels) & !grepl("bct", diamondlabels))

PA_ci_control <- ci_control %>% dplyr::filter(diamondlabels %in% names(scales_diff) & grepl("PA_", diamondlabels) & !grepl("bct", diamondlabels))
...
motiGirls <- regulations.df %>%
 tidyr::gather(key = Variable, value = Value, PA_amotivation_02_diff:ncol(.)) %>%
  filter(girl == "girl") %>% 
...
```{r motivation-histograms-gender-intervention-HRC}
regulations.df <- df %>% dplyr::filter(track == "HRC") %>% 
  dplyr::select(
  id,
  intervention,
  group,
...
# "PA_amotivation_diff")
# 
# PA_ci_girls <- ci_girls %>% dplyr::filter(diamondlabels %in% names(scales_diff) & 
#                                      grepl("PA_autonomous_diff", diamondlabels) | 
#                                      grepl("PA_controlled_diff", diamondlabels) | 
#                                      grepl("PA_amotivation_diff", diamondlabels))
# 
# PA_ci_girls <- PA_ci_girls[match(target, PA_ci_girls$diamondlabels), ]
# 
# PA_ci_boys <- ci_boys %>% dplyr::filter(diamondlabels %in% names(scales_diff) & 
#                                      grepl("PA_autonomous_diff", diamondlabels) | 
#                                      grepl("PA_controlled_diff", diamondlabels) | 
#                                      grepl("PA_amotivation_diff", diamondlabels))
# 
...
plot2 <- df %>% dplyr::select(id,
  ...
  theme(legend.position = "bottom")
# 
# PA_ci_intervention <- ci_intervention %>% dplyr::filter(diamondlabels %in% names(scales_diff) & 
#                                      grepl("PA_autonomous_diff", diamondlabels) | 
#                                      grepl("PA_controlled_diff", diamondlabels) | 
#                                      grepl("PA_amotivation_diff", diamondlabels))
# 
...
# 
# PA_ci_control <- ci_control %>% dplyr::filter(diamondlabels %in% names(scales_diff) & 
#                                      grepl("PA_autonomous_diff", diamondlabels) | 
#                                      grepl("PA_controlled_diff", diamondlabels) | 
#                                      grepl("PA_amotivation_diff", diamondlabels))
# 
...
plot1 <- df %>% dplyr::select(id,
  ...
  school,
  girl,
'Frequency-\nrelated BCTs' = PA_frequencyDependentBCT_diff,
'Agreement-\nrelated BCTs' = PA_agreementDependentBCT_diff)  %>%
  ...
  ggridges::geom_density_ridges(aes(x = Value, fill = paste(Variable, girl)), 
           alpha = .6, color = "black", from = -7, to = 7) +
  labs(x = "",
       y = "") +
  scale_y_discrete(expand = c(0.01, 0)) +
  scale_x_continuous(expand = c(0.01, 0)) +
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(PA_frequencyDependentBCT_diff, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.PA_frequencyDependentBCT_diff_2 <- sm.density.compare2(as.numeric(dens$PA_frequencyDependentBCT_diff), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(PA_frequencyDependentBCT_diff, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(PA_agreementDependentBCT_diff, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.PA_agreementDependentBCT_diff_2 <- sm.density.compare2(as.numeric(dens$PA_agreementDependentBCT_diff), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(PA_agreementDependentBCT_diff, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
```{r sm-frequencyDependentBCT-HRC}

# Create data frame
densplot <- df %>% dplyr::filter(track == "HRC")
levels(densplot$intervention) <- c("Control", "Intervention")
levels(densplot$girl) <- recode(densplot$girl, "boy" = "Boys", "girl" = "Girls")
...
bctGirls <- df %>% dplyr::select(id,
  ...
  school,
  girl,
'Reminded self of positive consequences' = PA_frequencyDependentBCT_01_diff,
'Has logged PA on paper' = PA_frequencyDependentBCT_02_diff,
'Has monitored PA on smartphone' = PA_frequencyDependentBCT_03_diff,
'Uses mnemonic cues' = PA_frequencyDependentBCT_04_diff,
'Has compared actual with goal' = PA_frequencyDependentBCT_05_diff,
'Has thought of relevance of PA' = PA_frequencyDependentBCT_06_diff,
'Has made changes at home' = PA_frequencyDependentBCT_07_diff,
'Has sought social support' = PA_frequencyDependentBCT_08_diff,
'Has evaluated why goal not reached' = PA_frequencyDependentBCT_09_diff) %>%
  tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>%
...
bctGirls <- df %>% dplyr::select(id,
  ...
  school,
  girl,
  'Has set goals' = PA_agreementDependentBCT_01_diff,
  'Has made plan' = PA_agreementDependentBCT_02_diff,
  'Plan made by other' = PA_agreementDependentBCT_03_diff,
  'Has reminder of plan' = PA_agreementDependentBCT_04_diff,
  'Has cut goals to subgoals' = PA_agreementDependentBCT_05_diff,
  'Has tried new PA options' = PA_agreementDependentBCT_06_diff,
  'Has thought of barriers' = PA_agreementDependentBCT_07_diff,
  'Has planned for barriers' = PA_agreementDependentBCT_08_diff,
  'Has thought of PA identity' = PA_agreementDependentBCT_09_diff,
  'Has fitted PA to life values' = PA_agreementDependentBCT_10_diff) %>%
  tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>%
  dplyr::mutate(Variable = forcats::fct_reorder(Variable, .x = Value, .desc = FALSE, 
                                                fun = mean, na.rm = TRUE)) %>% 
  dplyr::filter(girl == "girl") %>% 
...
bctGirls <- df %>% dplyr::filter(track == "HRC") %>% 
  dplyr::select(id,
  intervention,
  group,
  school,
...
bctBoys <- df %>% dplyr::filter(track == "HRC") %>% 
  dplyr::select(id,
  intervention,
  group,
  school,
...
bctInt <- df %>% dplyr::filter(track == "HRC") %>% 
  dplyr::select(id,
  intervention,
  group,
  school,
...
bctCont <- df %>% dplyr::filter(track == "HRC") %>% 
  dplyr::select(id,
  intervention,
  group,
  school,
...
```{r bct-determinants-nofill, eval = FALSE}
plot1 <- df %>% dplyr::select(id,
                              intervention,
                              group,
                              school,
                              ...
                              'PA perceived\nbehavioural control' = PA_perceivedBehaviouralControl_diff,
                              'PA self efficacy' = PA_selfefficacy_diff,
                              'PA perceived\nopportunities' = PA_opportunities_diff,
                              'PA descriptive\nnorm' = PA_descriptiveNorm_diff,
                              'PA injunctive\nnorm' = PA_injunctiveNorm_diff)  %>%
  # dplyr::select(noquote(order(colnames(.)))) %>% # Orders columns alphabetically
  tidyr::gather(key = Variable, value = Value, 6:ncol(.), factor_key = TRUE) %>%
  ggplot2::ggplot(aes(y = Variable)) +
  ggridges::geom_density_ridges(aes(x = Value, colour = paste(Variable, girl), 
                                    fill = paste(Variable, girl)), 
                                    alpha = 0, size = 0.75, from = -7, to = 7) +
  labs(x = "",
       y = "") +
  scale_y_discrete(expand = c(0.01, 0)) +
  scale_x_continuous(expand = c(0.01, 0)) +
...
"PA_perceivedBehaviouralControl_diff",
"PA_selfefficacy_diff",
"PA_opportunities_diff",
"PA_descriptiveNorm_diff",
"PA_injunctiveNorm_diff")

PA_ci_girls <- ci_girls %>% dplyr::filter(diamondlabels %in% names(scales_diff) & 
                                     grepl("PA_intention_diff", diamondlabels) | 
                                     grepl("PA_outcomeExpectations_diff", diamondlabels) | 
                                     grepl("PA_perceivedBehaviouralControl_diff", diamondlabels) | 
                                     grepl("PA_selfefficacy_diff", diamondlabels) | 
                                     grepl("PA_opportunities_diff", diamondlabels) | 
                                     grepl("PA_descriptiveNorm_diff", diamondlabels) | 
                                     grepl("PA_injunctiveNorm_diff", diamondlabels))
...
plot2 <- df %>% dplyr::select(id,
  ...
  ggridges::geom_density_ridges(aes(x = Value, colour = "black", 
                                    fill = paste(Variable, intervention)), 
                                    alpha = 0.75, size = 0.25, from = -7, to = 7, scale = 1) +
  labs(x = "",
       y = "") +
  scale_y_discrete(expand = c(0.01, 0), labels = NULL) +
  scale_x_continuous(expand = c(0.01, 0)) +
...
df %>% group_by(girl) %>% summarise(mean = mean(PA_descriptiveNorm_diff, na.rm = TRUE)) %>% dplyr::filter(girl == "girl") %>% dplyr::select(mean) < df %>% group_by(girl) %>% summarise(mean = mean(PA_descriptiveNorm_diff, na.rm = TRUE)) %>% dplyr::filter(girl == "boy") %>% dplyr::select(mean)

df %>% group_by(intervention) %>% summarise(mean = mean(PA_descriptiveNorm_diff, na.rm = TRUE)) %>% dplyr::filter(intervention == "1") %>% dplyr::select(mean) < df %>% group_by(intervention) %>% summarise(mean = mean(PA_descriptiveNorm_diff, na.rm = TRUE)) %>% dplyr::filter(intervention == "0") %>% dplyr::select(mean)
```
...
# 
# PA_ci_girls <- ci_girls %>% dplyr::filter(diamondlabels %in% names(scales_diff) & 
#                                      grepl("PA_actCop_diff", diamondlabels) | 
#                                      grepl("PA_intention_diff", diamondlabels) | 
#                                      grepl("PA_outcomeExpectations_diff", diamondlabels) | 
#                                      grepl("PA_perceivedBehaviouralControl_diff", diamondlabels) | 
#                                      grepl("PA_selfefficacy_diff", diamondlabels) | 
...
plot1 <- df %>% dplyr::select(id,
  ...
  school,
  girl,
'SB intention' = SB_intention_diff,
'SB outcome\nexpectations' = SB_outcomeExpectations_diff,
'SB self-efficacy\nand perceived\nbehavioural control' = SB_selfEfficacyperceivedBehaviouralControl_diff,
'SB descriptive\nnorm' = SB_descriptiveNorm_diff,
'SB injunctive\nnorm' = SB_injunctiveNorm_diff)  %>%
  # dplyr::select(noquote(order(colnames(.)))) %>% # Orders columns alphabetically
  tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>% 
  ggplot2::ggplot(aes(y = Variable)) +
  ggridges::geom_density_ridges(aes(x = Value,
...
# total_n <- df %>% select(mvpaAccelerometer_diff, sitLieAccelerometer_diff, padaysLastweek_diff, sitBreaksAccelerometer_diff) %>% 
#   na.omit(.) %>% 
#   summarise(n = n())
...
middle_x_percentile <- function(middle_percentile) {
  df %>%
  dplyr::filter(
    (df$mvpaAccelerometer_diff > quantile(df$mvpaAccelerometer_diff, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[1]]) &
    (df$mvpaAccelerometer_diff < quantile(df$mvpaAccelerometer_diff, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[2]]) &
    (df$sitLieAccelerometer_diff > quantile(df$sitLieAccelerometer_diff, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[1]]) &
    (df$sitLieAccelerometer_diff < quantile(df$sitLieAccelerometer_diff, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[2]]) & 
    (df$padaysLastweek_diff > quantile(df$padaysLastweek_diff, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[1]]) &
    (df$padaysLastweek_diff < quantile(df$padaysLastweek_diff, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[2]]) &
    (df$sitBreaksAccelerometer_diff > quantile(df$sitBreaksAccelerometer_diff, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[1]]) &
    (df$sitBreaksAccelerometer_diff < quantile(df$sitBreaksAccelerometer_diff, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[2]])) %>% 
    nrow(.)
}
  

getmiddle <- function(middlePerc) {                                       
df %>% dplyr::select(mvpaAccelerometer_diff, padaysLastweek_diff, sitLieAccelerometer_diff, sitBreaksAccelerometer_diff) %>% 
  na.omit(.) %>% 
  dplyr::summarise(
    n = n(), 
    middle = sum((mvpaAccelerometer_diff > 
                 (quantile(mvpaAccelerometer_diff, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[1]])) & 
                (mvpaAccelerometer_diff < 
                 (quantile(mvpaAccelerometer_diff, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[2]])) &
                (padaysLastweek_diff > 
                 (quantile(padaysLastweek_diff, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[1]])) & 
                (padaysLastweek_diff < 
                 (quantile(padaysLastweek_diff, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[2]])) &
                (sitLieAccelerometer_diff > 
                 (quantile(sitLieAccelerometer_diff, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[1]])) & 
                (sitLieAccelerometer_diff < 
                 (quantile(sitLieAccelerometer_diff, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[2]])) &
                (sitBreaksAccelerometer_diff > 
                 (quantile(sitBreaksAccelerometer_diff, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[1]])) & 
                (sitBreaksAccelerometer_diff < 
                 (quantile(sitBreaksAccelerometer_diff, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[2]]))
               )) %>% 
...
sum(df$mvpaAccelerometer_diff >= quantile(df$mvpaAccelerometer_diff, probs = c(prob1, prob2), na.rm = TRUE)[[1]] &
      df$mvpaAccelerometer_diff <= quantile(df$mvpaAccelerometer_diff, probs = c(prob1, prob2), na.rm = TRUE)[[2]] &
    df$padaysLastweek_diff >= quantile(df$padaysLastweek_diff, probs = c(prob1, prob2), na.rm = TRUE)[[1]] &
      df$padaysLastweek_diff <= quantile(df$padaysLastweek_diff, probs = c(prob1, prob2), na.rm = TRUE)[[2]] &
    df$sitLieAccelerometer_diff >= quantile(df$sitLieAccelerometer_diff, probs = c(prob1, prob2), na.rm = TRUE)[[1]] &
      df$sitLieAccelerometer_diff <= quantile(df$sitLieAccelerometer_diff, probs = c(prob1, prob2), na.rm = TRUE)[[2]] &
    df$sitBreaksAccelerometer_diff >= quantile(df$sitBreaksAccelerometer_diff, probs = c(prob1, prob2), na.rm = TRUE)[[1]] &
      df$sitBreaksAccelerometer_diff <= quantile(df$sitBreaksAccelerometer_diff, probs = c(prob1, prob2), na.rm = TRUE)[[2]],
    na.rm= TRUE) 
}    
...
# sum(df$mvpaAccelerometer_diff >= min(df$mvpaAccelerometer_diff, na.rm = TRUE) &
#       df$mvpaAccelerometer_diff <= max(df$mvpaAccelerometer_diff, na.rm = TRUE) &
#     df$padaysLastweek_diff >= min(df$padaysLastweek_diff, na.rm = TRUE) &
#       df$padaysLastweek_diff <= max(df$padaysLastweek_diff, na.rm = TRUE) &
#     df$sitLieAccelerometer_diff >= min(df$sitLieAccelerometer_diff, na.rm = TRUE) &
#       df$sitLieAccelerometer_diff <= max(df$sitLieAccelerometer_diff, na.rm = TRUE) &
#     df$sitBreaksAccelerometer_diff >= min(df$sitBreaksAccelerometer_diff, na.rm = TRUE) &
#       df$sitBreaksAccelerometer_diff <= max(df$sitBreaksAccelerometer_diff, na.rm = TRUE),
#     na.rm= TRUE) 
...
# Make variables numeric, and df as data frame
...
userfriendlyscience::CIBER(data = d_num,
      determinants = c(
        "actCop_diff",
        "agreementDependentBCT_diff",
        "amotivation_diff",
        "autonomous_diff",
        "big5_agreeableness_diff.1",
        "big5_conscientiousness_diff.1",
        "big5_extraversion_diff.1",
        "big5_neuroticism_diff.1",
        "big5_openness_diff.1",
        "controlled_diff",
        "descriptiveNorm_diff",
        "frequencyDependentBCT_diff",
        "goal_diff",
        "injunctiveNorm_diff",
        "intention_diff",
        "outcomeExpectations_diff",
        "opportunities_diff",
        "perceivedBehaviouralControl_diff",
        "selfefficacy_diff"),
      targets = c("paT1", "fatpct_diff"),
...
userfriendlyscience::CIBER(data = d_num,
      determinants = c(
        ...
        "outcomeExpectations_diff",
        "opportunities_diff",
        "seperceivedBehaviouralControl_diff"),
      targets = c("paT1", "fatpct_diff"),
      conf.level = list(means = 0.9999,
                        associations = 0.99)
      )
...
userfriendlyscience::CIBER(data = d_num,
  determinants = c(
  "agreementDependentBCT_01_diff",
  "agreementDependentBCT_02_diff",
  "agreementDependentBCT_03_diff",
  "agreementDependentBCT_04_diff",
  "agreementDependentBCT_05_diff",
  "agreementDependentBCT_06_diff",
  "agreementDependentBCT_07_diff",
  "agreementDependentBCT_08_diff",
  "agreementDependentBCT_09_diff",
  "agreementDependentBCT_10_diff",
  "frequencyDependentBCT_01_diff",
  "frequencyDependentBCT_02_diff",
  "frequencyDependentBCT_03_diff",
  "frequencyDependentBCT_04_diff",
  "frequencyDependentBCT_05_diff",
  "frequencyDependentBCT_06_diff",
  "frequencyDependentBCT_07_diff",
  "frequencyDependentBCT_08_diff",
  "frequencyDependentBCT_09_diff"),
   targets = c("MVPA", "SB"),
  leftAnchors = rep("", 19),
  rightAnchors = rep("", 19))
...
userfriendlyscience::CIBER(data = d_num,
  ...
  "intention_01_diff",
  "intention_02_diff",
  "selfefficacy_01_diff",
  "selfefficacy_02_diff",
  "perceivedBehaviouralControl_01_diff",
  "perceivedBehaviouralControl_02_diff",
  "perceivedBehaviouralControl_03_diff",
  "norm_01_diff",
  "norm_02_diff",
  "outcomeExpectations_01_diff",
  "outcomeExpectations_02_diff",
  "outcomeExpectations_03_diff",
  "outcomeExpectations_04_diff",
  "outcomeExpectations_05_diff",
  "outcomeExpectations_06_diff",
  "outcomeExpectations_07_diff",
  "outcomeExpectations_08_diff",
  "outcomeExpectations_09_diff",
  "outcomeExpectations_10_diff",
  "outcomeExpectations_11_diff",
  "outcomeExpectations_12_diff"),
   targets = c("MVPA", "SB"),
  leftAnchors = rep("", 21),
  rightAnchors = rep("", 21))
```
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(symptom_diff, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.symptom_diff_2 <- sm.density.compare2(as.numeric(dens$symptom_diff), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(symptom_diff, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
sympGirls <- df %>% dplyr::select(id,
  ...
  school,
  girl,
  "Neck and shoulder pain" = symptom_neckShoulderPain_diff,
  "Lower back pain" = symptom_lowerBackPain_diff,
  "Stomach ache" = symptom_stomachAche_diff,
  "Tension or nervousness" = symptom_tensionNervousness_diff,
  "Irritability or anger bursts" = symptom_irritabilityAngerbursts_diff,
  "Difficulty with sleep" = symptom_sleepDifficulty_diff,
  "Headache" = symptom_headAche_diff,
  "Tiredness or faintness" = symptom_tirednessFaintness_diff) %>%
 tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>%
  dplyr::filter(girl == "girl") %>% 
 ggplot2::ggplot(aes(x = Value, y = Variable, group = Variable)) +
  ggridges::geom_density_ridges2(aes(fill = Variable), stat = "binline", binwidth = 1, scale = 0.95) +
