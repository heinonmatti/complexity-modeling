...
# Centre the variable:
df_randeff_plot_rstanarm <- df %>% dplyr::mutate(mvpaAccelerometer_T3_centred = mvpaAccelerometer_T3 - mean(mvpaAccelerometer_T3, na.rm = TRUE))

m_1 <- stan_glmer(mvpaAccelerometer_T3_centred ~ (1 | group), data = df_randeff_plot_rstanarm, chains = 2, iter = 2000) 
pt <- ranef(m_1) 
...
# groupmeans <- df %>% dplyr::select(mvpaAccelerometer_T3, group) %>% group_by(group) %>% 
#   summarise(mean = mean(mvpaAccelerometer_T3, na.rm = TRUE)) %>% dplyr::filter(complete.cases(.))
...
# Centre the variable:
df_randeff_plot_brms <- df %>% dplyr::mutate(mvpaAccelerometer_T3_centred = 
                      mvpaAccelerometer_T3 - mean(mvpaAccelerometer_T3, na.rm = TRUE))

m_1 <- brms::bf(mvpaAccelerometer_T3_centred ~ (1 | group)) 
...
df %>% ggplot2::ggplot(aes(x = group, y = mvpaAccelerometer_T3_centred)) +
...
# Centre the variable:
df_randeff_plot_brms_autonomous <- df %>% dplyr::mutate(PA_autonomous_T3_centred = PA_autonomous_T3 - mean(PA_autonomous_T3, na.rm = TRUE))

m_1 <- brms::bf(PA_autonomous_T3_centred ~ (1 | group)) 
...
df_randeff_plot_brms_autonomous %>% ggplot2::ggplot(aes(x = group, y = PA_autonomous_T3_centred)) +
...
averageDayActivityPlot_gender <- df %>%  
  dplyr::select(girl, intervention, track,
                     "Sedentary behavior" = sitLieAccelerometer_T3, 
                     "Standing still" = standingAccelerometer_T3, 
                     "Light PA" = lpaAccelerometer_T3, 
                     "Moderate PA" = mpaAccelerometer_T3, 
                     "Vigorous PA" = vpaAccelerometer_T3) %>%
...
averageDayActivityPlot_gender <- df %>% 
  dplyr::mutate_at(vars(contains("Accelerometer"), -contains("weartime")), funs(./weartimeAccelerometer_T3)) %>% 
  dplyr::select(girl, intervention, track,
                     "Sedentary behavior" = sitLieAccelerometer_T3, 
                     "Standing still" = standingAccelerometer_T3, 
                     "Light PA" = lpaAccelerometer_T3, 
...
corrgram::corrgram(data.frame("Self-rep PA" = df$padaysLastweek_T3, 
                              "Accelerometer PA" = df$mvpaAccelerometer_T3, 
                              "Sedentary time" = df$sitLieAccelerometer_T3, 
                              "Interrupted sitting" = df$sitBreaksAccelerometer_T3), 
...
dens <- densplot %>% dplyr::select(weartimeAccelerometer_T3, girl) %>% 
...
# Make plot
sm.weartimeAccelerometer_T3_2 <- sm.density.compare2(as.numeric(dens$weartimeAccelerometer_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
...
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(weartimeAccelerometer_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
m_weartimeAccelerometer_trackgirl <- brms::bf(weartimeAccelerometer_T3 ~ (1 | track:girl)) %>% 
...
m_weartimeAccelerometer_trackintervention <- brms::bf(weartimeAccelerometer_T3 ~ (1 | track:intervention)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
df_for_models_nested <- df %>% 
  dplyr::select(track, weartimeAccelerometer_T3, girl, intervention) %>% 
...
df_fitted <- df_for_models_nested %>% 
  dplyr::mutate(fit = map(data, ~ lm(weartimeAccelerometer_T3 ~ 1, data = .x)))
...
df %>% dplyr::select(weartimeAccelerometer_T3, girl, trackSchool) %>% 
  group_by(girl, trackSchool) %>% 
  summarise(mean = mean(weartimeAccelerometer_T3, na.rm = TRUE),
            median = median(weartimeAccelerometer_T3, na.rm = TRUE),
            max = max(weartimeAccelerometer_T3, na.rm = TRUE),
            min = min(weartimeAccelerometer_T3, na.rm = TRUE),
            sd = sd(weartimeAccelerometer_T3, na.rm = TRUE),
            n = n()) %>% 
...
plot1 <- df %>% dplyr::select(id,
                              ...
                              girl,
                              Weartime = weartimeAccelerometer_T3)  %>%
...
testdf <- data.frame(PA = df$mvpaAccelerometer_T3, track = df$track, intervention = df$intervention)
...
MVPAgirl_df <- df %>% group_by(girl) %>% dplyr::select(girl, mvpaAccelerometer_T3) %>% summarise(`mean accelerometer-measured MVPA difference between girls and boys` = mean(mvpaAccelerometer_T3, na.rm = TRUE), `median accelerometer-measured MVPA difference between girls and boys` = median(mvpaAccelerometer_T3, na.rm = TRUE)) 

userfriendlyscience::meanDiff(df$girl, df$mvpaAccelerometer_T3)
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(mvpaAccelerometer_T3, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.mvpaAccelerometer_T3_2 <- sm.density.compare2(as.numeric(dens$mvpaAccelerometer_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(mvpaAccelerometer_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
m_mvpaAccelerometer_trackgirl <- brms::bf(mvpaAccelerometer_T3 ~ (1 | track:girl)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
m_mvpaAccelerometer_trackintervention <- brms::bf(mvpaAccelerometer_T3 ~ (1 | track:intervention)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
# To test the code with a single variable:
df_for_models_nested <- df %>% 
  dplyr::select(track, mvpaAccelerometer_T3, girl, intervention) %>% 
  dplyr::mutate(track = forcats::fct_recode(track, NULL = "Other")) %>%
  na.omit() %>% 
  dplyr::group_by(girl, intervention, track) %>% 
  tidyr::nest()
...
df_fitted <- df_for_models_nested %>% 
  dplyr::mutate(fit = map(data, ~ lm(mvpaAccelerometer_T3 ~ 1, data = .x)))
# Now there's a linear model for each combination in the data frame
...
df %>% dplyr::select(mvpaAccelerometer_T3, girl, trackSchool) %>% 
  group_by(girl, trackSchool) %>% 
  summarise(mean = mean(mvpaAccelerometer_T3, na.rm = TRUE),
            median = median(mvpaAccelerometer_T3, na.rm = TRUE),
            max = max(mvpaAccelerometer_T3, na.rm = TRUE),
            min = min(mvpaAccelerometer_T3, na.rm = TRUE),
            sd = sd(mvpaAccelerometer_T3, na.rm = TRUE),
            n = n()) %>% 
  DT::datatable() #papaja::apa_table()
...
m_padaysLastweek_trackgirl <- brms::bf(padaysLastweek_T3 ~ (1 | track:girl)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
m_padaysLastweek_trackintervention <- brms::bf(padaysLastweek_T3 ~ (1 | track:intervention)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(padaysLastweek_T3, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.padaysLastweek_T3_2 <- sm.density.compare2(as.numeric(dens$padaysLastweek_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(padaysLastweek_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
plotboys <- df %>% dplyr::select(id,
  ...
  dplyr::mutate(track = factor(track, levels = c("IT", "BA", "HRC", "Nur")),
                track = recode_factor(track, "IT" = "IT", "BA" = "BA", 
                                      "HRC" = "HRC", "Nur" = "Nur")) %>% 
  ggplot2::ggplot(aes(y = track)) +
  ggridges::geom_density_ridges2(aes(x = padaysLastweek_T3, fill = girl), stat = "binline", binwidth = 1, scale = 0.95, alpha = 0.6) +
...
plotgirls <- df %>% dplyr::select(id,
  ...
  dplyr::mutate(track = factor(track, levels = c("IT", "BA", "HRC", "Nur")),
                track = recode_factor(track, "IT" = "IT", "BA" = "BA", 
                                      "HRC" = "HRC", "Nur" = "Nur")) %>% 
  ggplot2::ggplot(aes(y = track)) +
  ggridges::geom_density_ridges2(aes(x = padaysLastweek_T3, fill = girl), stat = "binline", binwidth = 1, scale = 0.95) +
  scale_x_continuous(breaks = c(0:7), expand = c(0, 0), name = NULL) +
...
plotcontrol <- df %>% dplyr::select(id,
  ...
  dplyr::mutate(track = factor(track, levels = c("IT", "BA", "HRC", "Nur")),
                track = recode_factor(track, "IT" = "IT", "BA" = "BA", 
                                      "HRC" = "HRC", "Nur" = "Nur")) %>% 
  ggplot2::ggplot(aes(y = track)) +
  ggridges::geom_density_ridges2(aes(x = padaysLastweek_T3, fill = intervention), stat = "binline", binwidth = 1, scale = 0.95) +
  scale_x_continuous(breaks = c(0:7), expand = c(0, 0), name = NULL) +
  scale_y_discrete(expand = c(0.01, 0), name = "", labels = NULL) +
...
hist_padaysLastweek_girl_count <- df %>% dplyr::select(id,
  ...
  dplyr::mutate(track = factor(track, levels = c("IT", "BA", "HRC", "Nur")),
                track = recode_factor(track, "IT" = "IT", "BA" = "BA", 
                                      "HRC" = "HRC", "Nur" = "Nur"),
                padaysLastweek_T3 = as.integer(padaysLastweek_T3)) %>% 
  ggplot(aes(x=padaysLastweek_T3, fill=girl)) +
...
hist_padaysLastweek_girl_density <- df %>% dplyr::select(id,
  ...
  dplyr::mutate(track = factor(track, levels = c("IT", "BA", "HRC", "Nur")),
                track = recode_factor(track, "IT" = "IT", "BA" = "BA", 
                                      "HRC" = "HRC", "Nur" = "Nur"),
                padaysLastweek_T3 = as.integer(padaysLastweek_T3)) %>% 
  ggplot(aes(x=padaysLastweek_T3, y = ..density.., fill=girl)) +
  geom_histogram(binwidth=1, position="dodge", color = "black") +
  scale_x_continuous(breaks = 0:7) +
  labs(x = NULL) + 
...
hist_padaysLastweek_intervention_count <- df %>% dplyr::select(id,
  ...
  dplyr::mutate(track = factor(track, levels = c("IT", "BA", "HRC", "Nur")),
                track = recode_factor(track, "IT" = "IT", "BA" = "BA", 
                                      "HRC" = "HRC", "Nur" = "Nur"),
                padaysLastweek_T3 = as.integer(padaysLastweek_T3)) %>% 
  ggplot(aes(x=padaysLastweek_T3, fill=intervention)) +
  geom_histogram(binwidth = 1, position = "dodge", color = "black") +
  scale_x_continuous(breaks = 0:7) +
  labs(x = NULL) + 
...
hist_padaysLastweek_intervention_density <- df %>% dplyr::select(id,
  ...
  dplyr::mutate(track = factor(track, levels = c("IT", "BA", "HRC", "Nur")),
                track = recode_factor(track, "IT" = "IT", "BA" = "BA", 
                                      "HRC" = "HRC", "Nur" = "Nur"),
                padaysLastweek_T3 = as.integer(padaysLastweek_T3)) %>% 
  ggplot(aes(x=padaysLastweek_T3, y = ..density.., fill=intervention)) +
  geom_histogram(binwidth = 1, position = "dodge", color = "black") +
  scale_x_continuous(breaks = 0:7) +
  labs(x = NULL) + 
  scale_fill_manual(values = viridis::viridis(4, end = 0.8)[c(2, 4)], 
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(leisuretimeMvpaHoursLastweek_T3, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.leisuretimeMvpaHoursLastweek_T3_2 <- sm.density.compare2(as.numeric(dens$leisuretimeMvpaHoursLastweek_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(leisuretimeMvpaHoursLastweek_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
m_nordpaq_alternative_itemAccelerometer_trackgirl <- brms::bf(leisuretimeMvpaHoursLastweek_T3 ~ (1 | track:girl)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
m_nordpaq_alternative_itemAccelerometer_trackintervention <- brms::bf(leisuretimeMvpaHoursLastweek_T3 ~ (1 | track:intervention)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
# To test the code with a single variable:
df_for_models_nested <- df %>% 
  dplyr::select(track, leisuretimeMvpaHoursLastweek_T3, girl, intervention) %>% 
  dplyr::mutate(track = forcats::fct_recode(track, NULL = "Other")) %>%
  na.omit() %>% 
  dplyr::group_by(girl, intervention, track) %>% 
  tidyr::nest()
...
df_fitted <- df_for_models_nested %>% 
  dplyr::mutate(fit = map(data, ~ lm(leisuretimeMvpaHoursLastweek_T3 ~ 1, data = .x)))
# Now there's a linear model for each combination in the data frame
...
df %>% dplyr::select(leisuretimeMvpaHoursLastweek_T3, girl, trackSchool) %>% 
  group_by(girl, trackSchool) %>% 
  summarise(mean = mean(leisuretimeMvpaHoursLastweek_T3, na.rm = TRUE),
            median = median(leisuretimeMvpaHoursLastweek_T3, na.rm = TRUE),
            max = max(leisuretimeMvpaHoursLastweek_T3, na.rm = TRUE),
            min = min(leisuretimeMvpaHoursLastweek_T3, na.rm = TRUE),
            sd = sd(leisuretimeMvpaHoursLastweek_T3, na.rm = TRUE),
            n = n()) %>% 
  DT::datatable() #papaja::apa_table()
...
plot1 <- df %>% dplyr::select(id,
                              trackSchool = trackSchool,
                              girl,
                              nordpaq_alternative_item = leisuretimeMvpaHoursLastweek_T3)  %>%
  dplyr::filter(!is.na(trackSchool), !grepl('Other|NANA|BA4|HRC2|Nur2', trackSchool)) %>% # Drop categories with just few participants
  ggplot2::ggplot(aes(y = trackSchool)) +
...
m_sitBreaksAccelerometer_trackgirl <- brms::bf(sitBreaksAccelerometer_T3 ~ (1 | track:girl)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
m_sitBreaksAccelerometer_trackintervention <- brms::bf(sitBreaksAccelerometer_T3 ~ (1 | track:intervention)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(sitBreaksAccelerometer_T3, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.sitBreaksAccelerometer_T3_2 <- sm.density.compare2(as.numeric(dens$sitBreaksAccelerometer_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(sitBreaksAccelerometer_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(sitLieAccelerometer_T3, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.sitLieAccelerometer_T3_2 <- sm.density.compare2(as.numeric(dens$sitLieAccelerometer_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(sitLieAccelerometer_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
m_sitLieAccelerometer_trackgirl <- brms::bf(sitLieAccelerometer_T3 ~ (1 | track:girl)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
m_sitLieAccelerometer_trackintervention <- brms::bf(sitLieAccelerometer_T3 ~ (1 | track:intervention)) %>% 
  brms::brm(., data = df, chains = 4, iter = 4000, control = list(adapt_delta = 0.95))
...
df %>% dplyr::select(sitLieAccelerometer_T3, girl, track) %>% 
  group_by(girl, track) %>% 
  summarise(mean = mean(sitLieAccelerometer_T3, na.rm = TRUE),
            median = median(sitLieAccelerometer_T3, na.rm = TRUE),
            max = max(sitLieAccelerometer_T3, na.rm = TRUE),
            min = min(sitLieAccelerometer_T3, na.rm = TRUE),
            sd = sd(sitLieAccelerometer_T3, na.rm = TRUE),
            n = n()) %>% 
  DT::datatable() #papaja::apa_table()
...
df %>% dplyr::select(sitLieAccelerometer_T3, intervention, track) %>% 
...
df %>% dplyr::select(sitLieAccelerometer_T3, girl, intervention, track) %>% 
  dplyr::group_by(track) %>% 
  dplyr::arrange((sitLieAccelerometer_T3), .by_group = TRUE) %>% 
  na.omit() %>% 
  dplyr::filter(track != "Other") %>% 
  dplyr::filter(sitLieAccelerometer_T3 == max(sitLieAccelerometer_T3) | sitLieAccelerometer_T3 == min(sitLieAccelerometer_T3)) %>% 
  DT::datatable() #papaja::apa_table()
...
PA_ci_girls <- ci_girls %>% dplyr::filter(diamondlabels %in% names(scales_T3) & grepl("PA_", diamondlabels) & !grepl("bct", diamondlabels))

PA_ci_boys <- ci_boys %>% dplyr::filter(diamondlabels %in% names(scales_T3) & grepl("PA_", diamondlabels) & !grepl("bct", diamondlabels))

PA_ci_intervention <- ci_intervention %>% dplyr::filter(diamondlabels %in% names(scales_T3) & grepl("PA_", diamondlabels) & !grepl("bct", diamondlabels))

PA_ci_control <- ci_control %>% dplyr::filter(diamondlabels %in% names(scales_T3) & grepl("PA_", diamondlabels) & !grepl("bct", diamondlabels))
...
The participants in control and intervention groups, as well as boys and girls, reported on PA motivation similarly. Averages describe the variables poorly, as autonomous motivation is relatively flat and the two others are skewed. 
...
motiGirls <- regulations.df %>%
 tidyr::gather(key = Variable, value = Value, PA_amotivation_02_T3:ncol(.)) %>%
  filter(girl == "girl") %>% 
...
plot1 <- df %>% dplyr::select(id,
  ...
  school,
  girl,
  'PA autonomous\nmotivation' = PA_autonomous_T3,
'PA controlled\nmotivation' = PA_controlled_T3,
'PA amotivation' = PA_amotivation_T3)  %>%
...
PA_ci_girls <- ci_girls %>% dplyr::filter(diamondlabels %in% names(scales_T3) & 
                                     grepl("PA_autonomous_T3", diamondlabels) | 
                                     grepl("PA_controlled_T3", diamondlabels) | 
                                     grepl("PA_amotivation_T3", diamondlabels))

PA_ci_girls <- PA_ci_girls[match(target, PA_ci_girls$diamondlabels), ]

PA_ci_boys <- ci_boys %>% dplyr::filter(diamondlabels %in% names(scales_T3) & 
                                     grepl("PA_autonomous_T3", diamondlabels) | 
                                     grepl("PA_controlled_T3", diamondlabels) | 
                                     grepl("PA_amotivation_T3", diamondlabels))
...
PA_ci_intervention <- ci_intervention %>% dplyr::filter(diamondlabels %in% names(scales_T3) & 
                                     grepl("PA_autonomous_T3", diamondlabels) | 
                                     grepl("PA_controlled_T3", diamondlabels) | 
                                     grepl("PA_amotivation_T3", diamondlabels))
...
PA_ci_control <- ci_control %>% dplyr::filter(diamondlabels %in% names(scales_T3) & 
                                     grepl("PA_autonomous_T3", diamondlabels) | 
                                     grepl("PA_controlled_T3", diamondlabels) | 
                                     grepl("PA_amotivation_T3", diamondlabels))
...
plot1 <- df %>% dplyr::select(id,
  ...
  school,
  girl,
'Agreeableness' = big5agreeableness_T3,
'Conscientiousness' = big5conscientiousness_T3,
'Extraversion' = big5extraversion_T3,
'Neuroticism' = big5neuroticism_T3,
'Openness' = big5openness_T3)  %>%
...
plot1 <- df %>% dplyr::select(id,
  ...
  school,
  girl,
'Frequency-\nrelated BCTs' = PA_frequencyDependentBCT_T3,
'Agreement-\nrelated BCTs' = PA_agreementDependentBCT_T3)  %>%
  # dplyr::select(noquote(order(colnames(.)))) %>% # Orders columns alphabetically
  tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>% 
  ggplot2::ggplot(aes(y = Variable)) +
  ggridges::geom_density_ridges(aes(x = Value, fill = paste(Variable, girl)), 
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(PA_frequencyDependentBCT_T3, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.PA_frequencyDependentBCT_T3_2 <- sm.density.compare2(as.numeric(dens$PA_frequencyDependentBCT_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(PA_frequencyDependentBCT_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(PA_agreementDependentBCT_T3, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.PA_agreementDependentBCT_T3_2 <- sm.density.compare2(as.numeric(dens$PA_agreementDependentBCT_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(PA_agreementDependentBCT_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
bctGirls <- df %>% dplyr::select(id,
  ...
  school,
  girl,
'Reminded self of positive consequences' = PA_frequencyDependentBCT_01_T3,
'Has logged PA on paper' = PA_frequencyDependentBCT_02_T3,
'Has monitored PA on smartphone' = PA_frequencyDependentBCT_03_T3,
'Uses mnemonic cues' = PA_frequencyDependentBCT_04_T3,
'Has compared actual with goal' = PA_frequencyDependentBCT_05_T3,
'Has thought of relevance of PA' = PA_frequencyDependentBCT_06_T3,
'Has made changes at home' = PA_frequencyDependentBCT_07_T3,
'Has sought social support' = PA_frequencyDependentBCT_08_T3,
'Has evaluated why goal not reached' = PA_frequencyDependentBCT_09_T3) %>%
  tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>%
...
bctGirls <- df %>% dplyr::select(id,
  ...
  school,
  girl,
  'Has set goals' = PA_agreementDependentBCT_01_T3,
  'Has made plan' = PA_agreementDependentBCT_02_T3,
  'Plan made by other' = PA_agreementDependentBCT_03_T3,
  'Has reminder of plan' = PA_agreementDependentBCT_04_T3,
  'Has cut goals to subgoals' = PA_agreementDependentBCT_05_T3,
  'Has tried new PA options' = PA_agreementDependentBCT_06_T3,
  'Has thought of barriers' = PA_agreementDependentBCT_07_T3,
  'Has planned for barriers' = PA_agreementDependentBCT_08_T3,
  'Has thought of PA identity' = PA_agreementDependentBCT_09_T3,
  'Has fitted PA to life values' = PA_agreementDependentBCT_10_T3) %>%
  tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>%
  dplyr::mutate(Variable = forcats::fct_reorder(Variable, .x = Value, .desc = FALSE, 
                                                fun = mean, na.rm = TRUE)) %>% 
  dplyr::filter(girl == "girl") %>% 
...
plot1 <- df %>% dplyr::select(id,
                              ...
                              school,
                              girl,
                              'PA intention' = PA_intention_T3,
                              'PA outcome\nexpectations' = PA_outcomeExpectations_T3,
                              'PA perceived\nbehavioural control' = PA_perceivedBehaviouralControl_T3,
                              'PA self efficacy' = PA_selfefficacy_T3,
                              'PA perceived\nopportunities' = PA_opportunities_T3,
                              'PA descriptive\nnorm' = PA_descriptiveNorm_T3,
                              'PA injunctive\nnorm' = PA_injunctiveNorm_T3)  %>%
  # dplyr::select(noquote(order(colnames(.)))) %>% # Orders columns alphabetically
  tidyr::gather(key = Variable, value = Value, 6:ncol(.), factor_key = TRUE) %>%
  ggplot2::ggplot(aes(y = Variable)) +
...
"PA_perceivedBehaviouralControl_T3",
"PA_selfefficacy_T3",
"PA_opportunities_T3",
"PA_descriptiveNorm_T3",
"PA_injunctiveNorm_T3")

PA_ci_girls <- ci_girls %>% dplyr::filter(diamondlabels %in% names(scales_T3) & 
                                     grepl("PA_intention_T3", diamondlabels) | 
                                     grepl("PA_outcomeExpectations_T3", diamondlabels) | 
                                     grepl("PA_perceivedBehaviouralControl_T3", diamondlabels) | 
                                     grepl("PA_selfefficacy_T3", diamondlabels) | 
                                     grepl("PA_opportunities_T3", diamondlabels) | 
                                     grepl("PA_descriptiveNorm_T3", diamondlabels) | 
                                     grepl("PA_injunctiveNorm_T3", diamondlabels))
...
df %>% group_by(girl) %>% summarise(mean = mean(PA_descriptiveNorm_T3, na.rm = TRUE)) %>% dplyr::filter(girl == "girl") %>% dplyr::select(mean) < df %>% group_by(girl) %>% summarise(mean = mean(PA_descriptiveNorm_T3, na.rm = TRUE)) %>% dplyr::filter(girl == "boy") %>% dplyr::select(mean)

df %>% group_by(intervention) %>% summarise(mean = mean(PA_descriptiveNorm_T3, na.rm = TRUE)) %>% dplyr::filter(intervention == "1") %>% dplyr::select(mean) < df %>% group_by(intervention) %>% summarise(mean = mean(PA_descriptiveNorm_T3, na.rm = TRUE)) %>% dplyr::filter(intervention == "0") %>% dplyr::select(mean)
```
...
PA_ci_girls <- ci_girls %>% dplyr::filter(diamondlabels %in% names(scales_T3) & 
                                     grepl("PA_actCop_T3", diamondlabels) | 
                                     grepl("PA_intention_T3", diamondlabels) | 
                                     grepl("PA_outcomeExpectations_T3", diamondlabels) | 
                                     grepl("PA_perceivedBehaviouralControl_T3", diamondlabels) | 
                                     grepl("PA_selfefficacy_T3", diamondlabels) | 
...
plot1 <- df %>% dplyr::select(id,
  ...
  school,
  girl,
'SB intention' = SB_intention_T3,
'SB outcome\nexpectations' = SB_outcomeExpectations_T3,
'SB self-efficacy\nand perceived\nbehavioural control' = SB_selfEfficacyperceivedBehaviouralControl_T3,
'SB descriptive\nnorm' = SB_descriptiveNorm_T3,
'SB injunctive\nnorm' = SB_injunctiveNorm_T3)  %>%
  # dplyr::select(noquote(order(colnames(.)))) %>% # Orders columns alphabetically
  tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>% 
  ggplot2::ggplot(aes(y = Variable)) +
  ggridges::geom_density_ridges(aes(x = Value,
...
# total_n <- df %>% select(mvpaAccelerometer_T3, sitLieAccelerometer_T3, padaysLastweek_T3, sitBreaksAccelerometer_T3) %>% 
#   na.omit(.) %>% 
#   summarise(n = n())
...
middle_x_percentile <- function(middle_percentile) {
  df %>%
  dplyr::filter(
    (df$mvpaAccelerometer_T3 > quantile(df$mvpaAccelerometer_T3, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[1]]) &
    (df$mvpaAccelerometer_T3 < quantile(df$mvpaAccelerometer_T3, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[2]]) &
    (df$sitLieAccelerometer_T3 > quantile(df$sitLieAccelerometer_T3, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[1]]) &
    (df$sitLieAccelerometer_T3 < quantile(df$sitLieAccelerometer_T3, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[2]]) & 
    (df$padaysLastweek_T3 > quantile(df$padaysLastweek_T3, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[1]]) &
    (df$padaysLastweek_T3 < quantile(df$padaysLastweek_T3, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[2]]) &
    (df$sitBreaksAccelerometer_T3 > quantile(df$sitBreaksAccelerometer_T3, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[1]]) &
    (df$sitBreaksAccelerometer_T3 < quantile(df$sitBreaksAccelerometer_T3, probs = c(0.5 - middle_percentile/2, 0.5 + middle_percentile/2), na.rm = TRUE)[[2]])) %>% 
    nrow(.)
}
  

getmiddle <- function(middlePerc) {                                       
df %>% dplyr::select(mvpaAccelerometer_T3, padaysLastweek_T3, sitLieAccelerometer_T3, sitBreaksAccelerometer_T3) %>% 
  na.omit(.) %>% 
  dplyr::summarise(
    n = n(), 
    middle = sum((mvpaAccelerometer_T3 > 
                 (quantile(mvpaAccelerometer_T3, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[1]])) & 
                (mvpaAccelerometer_T3 < 
                 (quantile(mvpaAccelerometer_T3, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[2]])) &
                (padaysLastweek_T3 > 
                 (quantile(padaysLastweek_T3, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[1]])) & 
                (padaysLastweek_T3 < 
                 (quantile(padaysLastweek_T3, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[2]])) &
                (sitLieAccelerometer_T3 > 
                 (quantile(sitLieAccelerometer_T3, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[1]])) & 
                (sitLieAccelerometer_T3 < 
                 (quantile(sitLieAccelerometer_T3, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[2]])) &
                (sitBreaksAccelerometer_T3 > 
                 (quantile(sitBreaksAccelerometer_T3, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[1]])) & 
                (sitBreaksAccelerometer_T3 < 
                 (quantile(sitBreaksAccelerometer_T3, probs = c(0.5 - middlePerc/2, 0.5 + middlePerc/2))[[2]]))
               )) %>% 
...
sum(df$mvpaAccelerometer_T3 >= quantile(df$mvpaAccelerometer_T3, probs = c(prob1, prob2), na.rm = TRUE)[[1]] &
      df$mvpaAccelerometer_T3 <= quantile(df$mvpaAccelerometer_T3, probs = c(prob1, prob2), na.rm = TRUE)[[2]] &
    df$padaysLastweek_T3 >= quantile(df$padaysLastweek_T3, probs = c(prob1, prob2), na.rm = TRUE)[[1]] &
      df$padaysLastweek_T3 <= quantile(df$padaysLastweek_T3, probs = c(prob1, prob2), na.rm = TRUE)[[2]] &
    df$sitLieAccelerometer_T3 >= quantile(df$sitLieAccelerometer_T3, probs = c(prob1, prob2), na.rm = TRUE)[[1]] &
      df$sitLieAccelerometer_T3 <= quantile(df$sitLieAccelerometer_T3, probs = c(prob1, prob2), na.rm = TRUE)[[2]] &
    df$sitBreaksAccelerometer_T3 >= quantile(df$sitBreaksAccelerometer_T3, probs = c(prob1, prob2), na.rm = TRUE)[[1]] &
      df$sitBreaksAccelerometer_T3 <= quantile(df$sitBreaksAccelerometer_T3, probs = c(prob1, prob2), na.rm = TRUE)[[2]],
    na.rm= TRUE) 
}    
...
# sum(df$mvpaAccelerometer_T3 >= min(df$mvpaAccelerometer_T3, na.rm = TRUE) &
#       df$mvpaAccelerometer_T3 <= max(df$mvpaAccelerometer_T3, na.rm = TRUE) &
#     df$padaysLastweek_T3 >= min(df$padaysLastweek_T3, na.rm = TRUE) &
#       df$padaysLastweek_T3 <= max(df$padaysLastweek_T3, na.rm = TRUE) &
#     df$sitLieAccelerometer_T3 >= min(df$sitLieAccelerometer_T3, na.rm = TRUE) &
#       df$sitLieAccelerometer_T3 <= max(df$sitLieAccelerometer_T3, na.rm = TRUE) &
#     df$sitBreaksAccelerometer_T3 >= min(df$sitBreaksAccelerometer_T3, na.rm = TRUE) &
#       df$sitBreaksAccelerometer_T3 <= max(df$sitBreaksAccelerometer_T3, na.rm = TRUE),
#     na.rm= TRUE) 
...
userfriendlyscience::CIBER(data = d_num,
      determinants = c(
        "actCop_T3",
        "agreementDependentBCT_T3",
        "amotivation_T3",
        "autonomous_T3",
        "big5_agreeableness_T3.1",
        "big5_conscientiousness_T3.1",
        "big5_extraversion_T3.1",
        "big5_neuroticism_T3.1",
        "big5_openness_T3.1",
        "controlled_T3",
        "descriptiveNorm_T3",
        "frequencyDependentBCT_T3",
        "goal_T3",
        "injunctiveNorm_T3",
        "intention_T3",
        "outcomeExpectations_T3",
        "opportunities_T3",
        "perceivedBehaviouralControl_T3",
        "selfefficacy_T3"),
      targets = c("paT1", "fatpct_T3"),
...
userfriendlyscience::CIBER(data = d_num,
      determinants = c(
        ...
        "outcomeExpectations_T3",
        "opportunities_T3",
        "seperceivedBehaviouralControl_T3"),
      targets = c("paT1", "fatpct_T3"),
      conf.level = list(means = 0.9999,
                        associations = 0.99)
      )
...
userfriendlyscience::CIBER(data = d_num,
  determinants = c(
  "agreementDependentBCT_01_T3",
  "agreementDependentBCT_02_T3",
  "agreementDependentBCT_03_T3",
  "agreementDependentBCT_04_T3",
  "agreementDependentBCT_05_T3",
  "agreementDependentBCT_06_T3",
  "agreementDependentBCT_07_T3",
  "agreementDependentBCT_08_T3",
  "agreementDependentBCT_09_T3",
  "agreementDependentBCT_10_T3",
  "frequencyDependentBCT_01_T3",
  "frequencyDependentBCT_02_T3",
  "frequencyDependentBCT_03_T3",
  "frequencyDependentBCT_04_T3",
  "frequencyDependentBCT_05_T3",
  "frequencyDependentBCT_06_T3",
  "frequencyDependentBCT_07_T3",
  "frequencyDependentBCT_08_T3",
  "frequencyDependentBCT_09_T3"),
   targets = c("MVPA", "SB"),
  leftAnchors = rep("", 19),
  rightAnchors = rep("", 19))
...
userfriendlyscience::CIBER(data = d_num,
  ...
  "selfefficacy_01_T3",
  "selfefficacy_02_T3",
  "perceivedBehaviouralControl_01_T3",
  "perceivedBehaviouralControl_02_T3",
  "perceivedBehaviouralControl_03_T3",
  "norm_01_T3",
  "norm_02_T3",
  "outcomeExpectations_01_T3",
  "outcomeExpectations_02_T3",
  "outcomeExpectations_03_T3",
  "outcomeExpectations_04_T3",
  "outcomeExpectations_05_T3",
  "outcomeExpectations_06_T3",
  "outcomeExpectations_07_T3",
  "outcomeExpectations_08_T3",
  "outcomeExpectations_09_T3",
  "outcomeExpectations_10_T3",
  "outcomeExpectations_11_T3",
  "outcomeExpectations_12_T3"),
   targets = c("MVPA", "SB"),
  leftAnchors = rep("", 21),
  rightAnchors = rep("", 21))
```
...
## Girls vs. boys
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(symptom_T3, girl) %>% 
  dplyr::filter(complete.cases(.))
...
# Make plot
sm.symptom_T3_2 <- sm.density.compare2(as.numeric(dens$symptom_T3), 
                                               as.factor(dens$girl), 
                                               model = "equal",
                                               xlab = "", ylab = "",
                                               col = viridis::viridis(4, end  =  0.8)[c(3, 1)], 
...
## Intervention vs. control
# Choose only the variables needed and drop NA
dens <- densplot %>% dplyr::select(symptom_T3, intervention) %>% 
  dplyr::filter(complete.cases(.))
...
sympGirls <- df %>% dplyr::select(id,
  ...
  school,
  girl,
  "Neck and shoulder pain" = symptom_neckShoulderPain_T3,
  "Lower back pain" = symptom_lowerBackPain_T3,
  "Stomach ache" = symptom_stomachAche_T3,
  "Tension or nervousness" = symptom_tensionNervousness_T3,
  "Irritability or anger bursts" = symptom_irritabilityAngerbursts_T3,
  "Difficulty with sleep" = symptom_sleepDifficulty_T3,
  "Headache" = symptom_headAche_T3,
  "Tiredness or faintness" = symptom_tirednessFaintness_T3) %>%
 tidyr::gather(key = Variable, value = Value, 6:ncol(.)) %>%
  dplyr::filter(girl == "girl") %>% 
 ggplot2::ggplot(aes(x = Value, y = Variable, group = Variable)) +
  ggridges::geom_density_ridges2(aes(fill = Variable), stat = "binline", binwidth = 1, scale = 0.95) +
