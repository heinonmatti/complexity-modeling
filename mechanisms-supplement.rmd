...
df %>% 
  ...
  ggplot(aes(x = PA_intention_diff, y = paAccelerometer_diff)) + 
...
df %>% dplyr::filter(intervention == "1") %>% 
  dplyr::select(paAccelerometer_diff, PA_intention_diff) %>% 
  corrgram::corrgram(
  lower.panel = corrgram::panel.pie,
...
lm(data = df, paAccelerometer_diff ~ PA_intention_diff + track)

lm(data = df, paAccelerometer_diff ~ PA_intention_diff + track + intervention)
...
df %>% 
  dplyr::filter(intervention == "1") %>% 
  ggplot(aes(x = PA_autonomous_diff, y = paAccelerometer_diff)) + 
  geom_point(color = "blue") + 
  geom_smooth(method = "loess", fill = "blue") +
  geom_point(data = (df %>% dplyr::filter(intervention == "0")), color = "red") +
  geom_smooth(data = (df %>% dplyr::filter(intervention == "0")), color = "red", fill = "red", method = "loess" ) +
...
df %>% dplyr::filter(intervention == "1") %>% 
  dplyr::select(paAccelerometer_diff, PA_autonomous_diff) %>% 
  corrgram::corrgram(
  lower.panel = corrgram::panel.pie,
  upper.panel = corrgram::panel.conf,
  main = "intervention group")
...
lm(data = df, paAccelerometer_diff ~ PA_autonomous_diff + track)

lm(data = df, paAccelerometer_diff ~ PA_autonomous_diff + track + intervention)
```
...
data_moderate_PA_and_autonomous <- df %>% 
  dplyr::mutate(moderate_PA_autonomous = ifelse(test = 
      df$paAccelerometer_T1 >= quantile(df$paAccelerometer_T1, probs = c(0.5 - i, 0.5 + i), na.rm = TRUE)[[1]] &
      df$paAccelerometer_T1 <= quantile(df$paAccelerometer_T1, probs = c(0.5 - i, 0.5 + i), na.rm = TRUE)[[2]] &
...
ci_low <- cor.test(data_moderate_PA_and_autonomous$paAccelerometer_diff, 
                   data_moderate_PA_and_autonomous$PA_autonomous_diff, 
                   method = "pearson", conf.level = 0.95)$conf.int[1]
ci_high <- cor.test(data_moderate_PA_and_autonomous$paAccelerometer_diff, 
                   data_moderate_PA_and_autonomous$PA_autonomous_diff, 
...
data_moderate_PA_and_sePbc <- df %>% 
  ...
  dplyr::select(paAccelerometer_diff, PA_sePbc_diff)

correlation <- cor(data_moderate_PA_and_sePbc, use = "pairwise.complete.obs", method = "kendall")[1, 2]
ci_low <- cor.test(data_moderate_PA_and_sePbc$paAccelerometer_diff, 
                   data_moderate_PA_and_sePbc$PA_sePbc_diff, 
                   method = "pearson", conf.level = 0.95)$conf.int[1]
ci_high <- cor.test(data_moderate_PA_and_sePbc$paAccelerometer_diff, 
                   data_moderate_PA_and_sePbc$PA_sePbc_diff, 
                   method = "pearson", conf.level = 0.95)$conf.int[2]
...
```{r}
quantile(df$paAccelerometer_T1, probs = c(0.15, 0.85), na.rm = TRUE)
quantile(df$PA_autonomous_T1, probs = c(0.15, 0.85), na.rm = TRUE)


data_moderate_PA_and_autonomous <- df %>% 
  dplyr::mutate(moderate_PA_autonomous = ifelse(test = 
    df$paAccelerometer_T1 >= quantile(df$paAccelerometer_T1, probs = c(0.15, 0.85), na.rm = TRUE)[[1]] &
      df$paAccelerometer_T1 <= quantile(df$paAccelerometer_T1, probs = c(0.15, 0.85), na.rm = TRUE)[[2]] &
...
```{r}
quantile(df$paAccelerometer_T1, probs = c(0.2, 0.8), na.rm = TRUE)
quantile(df$PA_autonomous_T1, probs = c(0.2, 0.8), na.rm = TRUE)


data_moderate_PA_and_autonomous <- df %>% 
  dplyr::mutate(moderate_PA_autonomous = ifelse(test = 
    df$paAccelerometer_T1 >= quantile(df$paAccelerometer_T1, probs = c(0.2, 0.8), na.rm = TRUE)[[1]] &
      df$paAccelerometer_T1 <= quantile(df$paAccelerometer_T1, probs = c(0.2, 0.8), na.rm = TRUE)[[2]] &
...
```{r}
quantile(df$paAccelerometer_T1, probs = c(0.25, 0.75), na.rm = TRUE)
quantile(df$PA_autonomous_T1, probs = c(0.25, 0.75), na.rm = TRUE)


data_moderate_PA_and_autonomous <- df %>% 
  dplyr::mutate(moderate_PA_autonomous = ifelse(test = 
    df$paAccelerometer_T1 >= quantile(df$paAccelerometer_T1, probs = c(0.25, 0.75), na.rm = TRUE)[[1]] &
      df$paAccelerometer_T1 <= quantile(df$paAccelerometer_T1, probs = c(0.25, 0.75), na.rm = TRUE)[[2]] &
...
lm(data = data_moderate_PA_and_autonomous, paAccelerometer_diff ~ PA_autonomous_diff + track)

lm(data = data_moderate_PA_and_autonomous, paAccelerometer_diff ~ PA_autonomous_diff + track + intervention)
```

### middle 40%

```{r}
quantile(df$paAccelerometer_T1, probs = c(0.3, 0.7), na.rm = TRUE)
quantile(df$PA_autonomous_T1, probs = c(0.3, 0.7), na.rm = TRUE)


data_moderate_PA_and_autonomous <- df %>% 
  dplyr::mutate(moderate_PA_autonomous = ifelse(test = 
    df$paAccelerometer_T1 >= quantile(df$paAccelerometer_T1, probs = c(0.3, 0.7), na.rm = TRUE)[[1]] &
      df$paAccelerometer_T1 <= quantile(df$paAccelerometer_T1, probs = c(0.3, 0.7), na.rm = TRUE)[[2]] &
...
```{r}
quantile(df$paAccelerometer_T1, probs = c(0.35, 0.65), na.rm = TRUE)
quantile(df$PA_autonomous_T1, probs = c(0.35, 0.65), na.rm = TRUE)


data_moderate_PA_and_autonomous <- df %>% 
  dplyr::mutate(moderate_PA_autonomous = ifelse(test = 
    df$paAccelerometer_T1 >= quantile(df$paAccelerometer_T1, probs = c(0.35, 0.65), na.rm = TRUE)[[1]] &
      df$paAccelerometer_T1 <= quantile(df$paAccelerometer_T1, probs = c(0.35, 0.65), na.rm = TRUE)[[2]] &
...
meandiffs <- df %>% select(contains("diff"), -contains("Difficulty"), -contains("Accelerometer"), -contains("sitBreaks"), intervention) %>% 
...
# fitdist_input <- df %>% dplyr::select(paAccelerometer_diff) %>% na.omit(.) %>% as.vector()
...
fitdist_input <- df %>% dplyr::select(paAccelerometer_diff) %>% na.omit(.) %>% as.vector(.) %>% abs(.)
attributes(fitdist_input) <- NULL
fitdist_input <- fitdist_input[[1]]
fitdistrplus::descdist(fitdist_input, discrete = FALSE, boot = 1000)
...
fitdist_input <- df %>% 
  dplyr::filter(intervention == 1) %>% 
  dplyr::select(paAccelerometer_diff) %>% na.omit(.) %>% as.vector(.) %>% abs(.)
attributes(fitdist_input) <- NULL
fitdist_input <- fitdist_input[[1]]
fitdistrplus::descdist(fitdist_input, discrete = FALSE, boot = 1000)
...
# fitdist_input <- df %>% dplyr::select(sitBreaks_diff) %>% na.omit(.) %>% as.vector()
# attributes(fitdist_input) <- NULL
# fitdist_input <- fitdist_input[[1]]
# fitdistrplus::descdist(fitdist_input, discrete = FALSE, boot = 1000)
# mtext("Raw values of differences in accelerometer-measured interruptions to sitting", padj = 6, side = 1)

fitdist_input <- df %>% dplyr::select(sitBreaks_diff) %>% na.omit(.) %>% as.vector(.) %>% abs(.)
attributes(fitdist_input) <- NULL
fitdist_input <- fitdist_input[[1]]
fitdistrplus::descdist(fitdist_input, discrete = FALSE, boot = 1000)
...
ggsave("nonlinplot.jpg", fullplot, width = 8.5, height = 11)

sum(!is.na(maps$every10th_intervention))
```
